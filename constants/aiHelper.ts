...
export function calculateAiAnalytics(from: Hub, to: Hub): AiAnalyticsResult {
  // Простой расчет расстояния по формуле гаверсинусов (в км)
  const R = 6371;
  ...
  return {
    ...
    activeRisks: distanceKm > 700 ? [
      { type: 'traffic', severity: 'medium', message: 'Возможны плотные заторы на объездных участках крупный городов.' }
    ] : []
  };
}